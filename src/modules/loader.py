        ...
        if root != '.':  #ignore current directory
            for f in files:
                if len(root.lstrip('./').split(os.path.sep)) == 2:
                    module_dirname = root.lstrip('./').split(os.path.sep)[1] #.replace('/','.')
                else:
                    continue
                ...
                if f == f"{module_dirname}_module.py":  # <--- load this!
                    ...
                    print(f"classname: {classname}, to import: {modname}.{module_dirname}_module")
                    m = importlib.import_module(f"{modname}.{module_dirname}_module")
